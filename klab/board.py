...
@cached
def value_players(exch: dict | None = None) -> tuple[pd.DataFrame, dict, dict]:
    ...
    pool_rp = float(top["roto_points"].sum() - repl_rp * n_rostered)
    dollars_above_min = C.N_TEAMS * C.BUDGET - n_rostered * 1.0
    usd_per_rp = dollars_above_min / pool_rp

    def dollars(rp):
        return ((rp - repl_rp) * usd_per_rp + 1.0).clip(lower=0.0)

    players = base_players.copy()
    players["rp_above_repl"] = players["roto_points"] - repl_rp
    players["redraft_value"] = dollars(players["roto_points"])
...
def value_2028(exch: dict, meta: dict, saves_2027: pd.Series) -> pd.DataFrame:
    ...
    out["redraft_value_2028"] = (
        (out["roto_points_2028"] - meta["replacement_rp"]) * scale + 1.0).clip(lower=0.0)
    return out


def build_board(exch: dict | None = None) -> tuple[pd.DataFrame, dict, dict]:
    players, exch, meta = value_players(exch)
