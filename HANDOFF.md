...
3. **The role cap is still a patch.** Uncapped, the model predicts $55 for a hitter against a $45 ceiling; 3 players sit at the cap. Both designed-in fixes lost on LOSO (concave transforms, monotone GBM), so this needs a different idea, not another sweep.
4. **Lower-tail band coverage does not transfer.** Conformal calibration fixed the upper end exactly (19.6% against a 20% target) and left the lower at 25%. 55.4% overall against a nominal 60%.
