...
2. **Alternative exchange-rate estimators (Josh's addition, confirmed in scope with item 1 on 2026-09-09).** The current estimator is two stacked OLS fits: DELIVERED points-on-salary per season, then $/pt-on-keeper-count across n=5 seasons, predicted at 100 keepers (`fit_exchange_rate` in `klab/board.py`, `EXCHANGE_BASIS = "keeper_adjusted"`). Candidates to build and score, first two from Josh's own framing:
   - **pooled pick-level regression over all 2024-26 purchases with a season term** (fixed effects, or keeper-count as a pick-level covariate): uses all ~404 picks AND captures the regime drift, with standard errors from n=404 instead of a five-point second stage. Likely the best-identified member of the family; a straight pool with no season term is the known-wrong baseline to show beside it ($/pt averaged across three different worlds);
   - **fit on ex-ante PROJECTED points instead of delivered points**: the market bids on projections, not outcomes, so the delivered-points hedonic is one modelling choice, not the only one. Needs the rewound projections in `data/` (#67 makes 2024-26 ex-ante lines available);
   - tiered exchange (separate $/pt by salary or points tier; the #75/#2 evidence is already tiered, so near-free);
   - window variants (2024-25 only, 2024-26 pooled, 2026 alone: the "recent" basis exists in config, never surfaced);
   - partial pooling (shrink per-season fits toward the pooled $5.83 instead of choosing a basis);
   - robust or median regression against the $1 mass and right skew.
   Identification caveat no new fit escapes: keeper count and calendar time are collinear at n=5 (#19), so anything trend-projected is a different extrapolation of the same five points, not new information; the decision test is the only arbiter.
