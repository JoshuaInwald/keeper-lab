...
`auction.auction_sample()` pairs every auction purchase (677 over 2022-26; 671 names resolved by `io.NameResolver`) with the roto points that player delivered that season (`auction.score_season()`); busts and never-played picks stay in at the price paid. `auction.regress()` fits `roto_points ~ salary` (production on price, never the reverse) with robust errors. `board.fit_exchange_rate()` under `EXCHANGE_BASIS = "keeper_adjusted"` fits $/point and intercept per season against keepers removed (`KEEPERS_REMOVED`: 25, 28, 29, 29, 100) and predicts at `KEEPERS_EXPECTED_2027 = 100`. Current: `roto_points = 3.98 + 0.100 x $`, $10.00 per point (quote `out/model_params.json`, it moves on every rebuild), n=404 on `AUCTION_SEASONS` 2024-26. `auction.spec_battery()` writes the alternative specifications to `out/auction_regression_battery.csv`. The full estimator family (pick-level with keeper-count covariate, pooled windows, partial pooling, median regression, projected-points, two piecewise curves) is `klab/exchange.py`, scored by decisions in `scripts/exchange_lab.py` (FINDINGS #82); the app ships four of them as the "auction fit" dropdown, `keep_cost()` in `klab/board.py` being the one place a fit becomes a dollar.
