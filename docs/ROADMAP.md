...
**Status 2026-09-08: Steps 0, 1, 1b and 2 are built (`docs/FINDINGS.md` #56). Steps 3, 4 and 5 are not.** The price model beats both baselines out of sample (MAE 5.76 against 6.84 prior-salary and 7.20 comps) and fails its own P20-P80 coverage test (38.4% against a nominal 60%), so `market_price` is a real column holding NaN. Two claims in the diagnosis below did not survive contact with the data and are corrected in #56: the 25% no-prior-line share is 80% censoring of 2022, and the 2022-2025 keeper files are not keeper submissions at all.
