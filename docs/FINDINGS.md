...
### 81. The UI review: the presentation layer swept bottom-up, four fixes, 2.4 MB dropped, four new guards
2026-09-09, the process of `docs/MODEL-REVIEW.md` pointed at `app/template.html` / `scripts/build_app.py` / `app/verify.mjs`; full detail in `docs/UI-REVIEW.md`. No number the model computes changed; the committed app and reference files did.

**81.1 The League tab went blank on two of three projection bases.** `build_payload()` stamped `points_2026` onto the top-level `teams` alias and onto `positional_variants["on"]` but never onto `"off"`, which for the two subprocess-built bases is a separate object after the JSON round-trip, and `applyVariant()` reads only `positional_variants`. Verified in the committed build: `off.points_2026 = null` for "projection" and "actuals", so the basis selector emptied "Points now" for all ten teams. Fixed by stamping both settings; the aliases that hid it are gone (81.3); `test_invariants.py` now asserts the field per basis and setting.

**81.2 A split two-way player was half a player in the JS trade path.** #80.4's Python fix never reached the browser: `byId` was last-row-wins, so the Trade tab priced Ohtani at his pitcher row alone ($4.95 of $83.66 keep_value), the picker listed him twice, and clicking his hitter row opened the pitcher drawer. Fixed with `rowsById` + `assetVal()` mirroring `klab.trade._TWO_WAY_SUM_COLS` (values sum, the one contract counts once), picker dedupe with an H/P label, and a role argument on `showPlayer()`. Guarded end-to-end in `verify.mjs`.

**81.3 The payload shipped every board twice; 2.4 MB was dead.** `basis_variants[b].{cols,board,fa,teams,constants}` (1,199,931 B) were byte-identical unread copies of `positional_variants.off`; the top-level board/fa/teams/constants/estimates aliases (~1,217,000 B) were read only for the initial render. Startup now calls `applyVariant()` like any basis switch, `cols` ships once, and `years_controlled`/`surplus_redraft`/`current_points` (computed, serialised, never read) are gone. Built file 7,669,157 B to 5,245,232 B (-31.6%) with one column ADDED (81.5). The structure, not just the bytes, is the fix: no screen can read a stale alias again, which is what 81.1 was.

**81.4 Sixteen stale user-facing strings, the #71.1 class.** The worst: five places still described role-blind replacement ("the 230th-best player", one number on the League card, the drawer subtracting the pitcher bar from every hitter's reconciliation row); the footer claimed "+/-34% per category" against the denominator panel's own +/-13-18% and credited the value scales with the price model's n=677; `CONST_HELP.auction_intercept` rendered "docs/FINDINGS.md #27" in a tooltip; the trade panel showed "#40"; the auction-estimate note claimed no age data exists anywhere (stale since the Chadwick ingest); one help string invited the reader to slide a control that does not exist. All fixed; the drawer's reconciliation now uses the row's own bar (`roto_points - rp_above_repl`), exact per role and under positional adjustment. `replacement_by_role` ships in constants for the two panels that state the bar.

**81.5 The one client-side dollar computation is gone.** `replV()`'s full-season branch derived a dollar in the browser from the intercept; `keep_value_ft` now ships from `klab/board.py` (byte-identical arithmetic, serialised server-side). The JS sim's shock scale likewise now reads `finish_sim.shock_scale` from the payload instead of a hard-coded 0.35.

**81.6 verify.mjs: 17 checks to 21, each on a reader-found class with no prior guard.** Phone hide-list checked by field name at 390px (#73's fourth edit, previously unguarded); the raw help-string audit (leaks, jargon, essay openers) closing the only-scans-current-DOM gap that let #27 through; bands-bracket-their-headline over all 277 rows (0 misses; #80.2's class); the split-asset convention (81.2); and the trade-suggestion check now fails on an empty payload instead of passing silently, the stale-build failure CLAUDE.md warns about. Diagnostics recorded, not fixed (UI-REVIEW section 3): the Trade tab's redraft-basis "delta surplus" label collision with the board's keep-basis column (matches `evaluate_trade`; now labeled; whether the server convention should move to KEEP_BASIS is a model question), split-player shading on the Intuition tab, and the 3.1 MB of visible auction comps as the next payload lever.

### 82. The exchange-rate estimator family, scored by decisions: the level is bracketed, the ordering is not in doubt, and no curve survives its gates
2026-09-09, the HANDOFF variant-set session. `klab/exchange.py` fits fourteen estimators of the $-per-roto-point rate; `scripts/exchange_lab.py` scores each three ways: keep/cut flips on the 2027 board against the shipped fit, the #69 backtest re-run with the estimator refit on strictly pre-season data, and a leave-one-season-out shape test. Outputs in `out/exchange_variants.csv`, `exchange_backtest.csv`, `exchange_loso.csv`, `anchor_variants.csv`.

**82.1 The best-identified estimator validates the shipped level, and repairs #19's "right number for the wrong reason".** A single pick-level regression over all 677 purchases 2022-26 with keeper count as a covariate (rp ~ salary + k + salary x k, read at k = 100) gives $10.38/pt against the shipped two-stage $10.00, with ZERO keep/cut flips and a real standard error at last: +/-$2.76 from n = 677, in place of a five-point second stage that had none. The identification caveat stands (keeper count and calendar time are collinear, #19; this is the same extrapolation with honest errors), but the family's well-identified member and 2026 alone ($9.92, zero flips) both land on the shipped answer. The pooled family is a different world: pooled 2024-26 $7.04 (20 flips), 2022-26 $5.41 (20), 2024-25 $6.34 (27), partial pooling shrunk toward the pool $7.10 (20). A median-regression first stage, robust to the $1 point mass, steepens the trend to $16.36 (11 flips, keeps 128). The menu brackets the level $7.04-$16.36; the decision-relevant flips concentrate in the pooled members that ignore the regime.

**82.2 Rewound, the shipped trend architecture fails catastrophically and the pick-level form does not.** Refit only on pre-2026 auctions (k = 25-29) and asked to predict the k = 100 regime, the two-stage trend advises keeping NOBODY: 0 of 134 keeps, -$868.0 against keep-everything, because the five-point intercept trend extrapolates to ~34 free roto points. The same rewind of the pick-level form scores +$111.8 on clean-label 2026, BEATING the backtest's own pooled-prior baseline (+$78.6) at higher accuracy (66.4% vs 64.9%); on the biased-label seasons it trails (2024 +$119.4 vs +$206.0, 2025 -$83.7 vs -$56.2), so no member wins the backtest outright. The projected-points hedonic (Josh's hypothesis: the market bids on projections, not outcomes) is refuted where it can be tested: fit on ex-ante blended Marcel points it keeps 11-34% of decisions and scores -$331.6 / -$253.9 on 2025/2026, the worst of the family. Default stays `keeper_adjusted`: the alternative that behaves better ex ante agrees with it to zero flips, so there is no decision to buy by switching.

**82.3 The value-side curve, tried and declined on its own gates.** The piecewise fit of delivered points on salary (knots $6/$16/$31, keeper count as a level term) is genuinely non-linear in-sample: segment marginal rates $5.12 / $7.03 / $4.01 / $24.55 per point. But the curvature runs the OPPOSITE direction from the revealed-price instruments: delivered elite points are scarce, so the inverse cost curve is CONVEX at the top (Skubal keep_value $110.86 against the linear $86.93), where #56.5/#57.6 show revealed PRICES concave at the top ($38.54/$34.31). Both are true: owners never pay past the $45 ceiling precisely because delivered points at high price are unreliable; a replacement-cost curve built on delivered points therefore steepens rather than bends. And none of it transfers: LOSO mean held-out RMSE 3.402 linear vs 5.189 spline+k vs 6.851 linear+k (the k terms detonate on the 2026 fold, RMSE 20.5 -- #19 made measurable), 23 flips on the board, and the rewound 2026 backtest scores it keep-nobody like the two-stage. The tiered two-stage (per-season segment rates trended on keeper count) degenerates outright: the top segment is ~7 picks per season and supports no trend, which `klab/exchange.py` raises on rather than ships. HANDOFF item 1 is answered in the negative with evidence; the linear scale stands.

**82.4 The replacement-anchor family was half wired, and the anchors move prices, not calls.** Under `POOL_RULE = "slot_role"` (#70) the "medium" anchor silently no-op'd (the per-role branch overwrote it) and "high" quietly reverted the calibration pool to role-blind top-230. Recoded so the POOL is the fieldable 140/90 under every anchor and `WAIVER_VALUE` moves only the BAR: "low" the fieldable minimum per role (5.121/3.526, unchanged default), "medium" #62's 300th-best split by the fieldable share (183rd hitter 4.733 / 117th pitcher 3.050, $/rp 6.213 to 4.952), "high" the measured FA-pickup level role-blind (5.04/5.04, $/rp 7.909). Keep/cut flips under every anchor: ZERO, measured, because `KEEP_BASIS = "replacement"` prices keeps off the exchange rate, which never sees the anchor (#69's pricing/decision split, now visible as a toggle). Redraft prices move by up to $12 (median $1-2.2); the #62 evidence tie stands and the dropdown ships all three.

**82.5 Shipped as two header dropdowns via column overlays, not board copies.** "auction fit" (keeper_adjusted default, pick_level, pooled_2426, median_two_stage) overlays the eleven keep-scale columns; "free-player bar" (low default, medium, high) overlays the eight redraft-scale columns per positional setting; constants and team sums patch with them, keep flags follow the shipped default only (#76), and the bootstrap bands are recomputed per variant server-side so every band brackets the headline it sits under (#80.2's contract, held under every dropdown state). The failed curves are deliberately NOT in the menu (the LOSO gate). Auction comps trimmed 8 to 5 per player (the designated relief valve, UI-REVIEW section 3) to pay for the overlays. Tiered inflation ships beside the scalar: predicted auction price over roster worth is 0.85x for the top 50, 1.00x for the mid 70, 2.19x for depth -- the ceiling holds stars below worth and the leftover money chases depth, so the scalar 1.28x describes nobody. `verify.mjs` 21 checks to 23 (each dropdown: moves its scale, moves zero keep flags, survives the other toggles, round-trips exactly).
