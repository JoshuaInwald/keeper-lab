...
- `docs/`: METHODS (how), FINDINGS (results, numbered #1-#63, cited from code comments), ROADMAP (next), WORKFLOWS (recipes), SESSION-LOG (history), ASSESSMENT-BRIEF (the current phase), constitution.txt (league rules).
...
- Build: `PYTHONPATH=.:scripts python3 scripts/run_all.py` (~3 s cold). Tests: `PYTHONPATH=. python3 -m pytest tests/ -q` (64 tests, 3-6 min; `test_build_payload` dominates). App check: `cd app && npm i playwright@1.56.1 && cd .. && node app/verify.mjs` (15 checks; JS re-implementation must match pandas on 25 quantities).
