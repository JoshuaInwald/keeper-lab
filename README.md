...
2. **Valuation-curve toggles, then a UI review.** Josh's queued ask: non-linear dollar curves (splines at the tails first) as UI toggles beside the existing ones, then the MODEL-REVIEW process applied to the app layer. Full scope in `HANDOFF.md` "Next session: start here".
...
4. **November onward: the projection refresh calendar** (`docs/MODEL-REVIEW.md` section 5): Steamer 2027 ~Nov (first 2026-aware system), fresh ZiPS Nov-Jan, ATC ~Feb. #69 localised the binding constraint to the projection; nothing 2026-aware exists anywhere before then. (The shipped decision rule itself has now been re-scored: it edges the owners on 2026 dollars, #79.1.)
5. **Watch the two consequences of #70 that still carry weight:** `MAX_KEEPERS` binds for 6 of 10 teams, and `keep_value` runs rich at the top (which item 2's non-linear curves attack directly). The keeper-count equilibrium is closed by measurement: two flips (#79.35).
