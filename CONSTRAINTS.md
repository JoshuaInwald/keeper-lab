...
- `out/keeper_lab.html`, `model_params.json`, `keeper_board_2027.csv`, `auction_sample.csv`, `price_features.csv`, `keeper_decisions.csv`, `price_model_holdout.csv`, `valuation_comparison.csv`, `age_persistence.csv`, `waiver_moves.csv`, `decision_audit.csv`, `trade_suggestions.json`, `app_reference.json`, `audit.txt`, `denominators_by_season.csv`, `sensitivity_*.csv` are committed on purpose. Everything else `run_all.py` writes is gitignored; do not add it back.
